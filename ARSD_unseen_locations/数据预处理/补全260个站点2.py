...
data = np.load(r'/mnt/hdd1/zhanghonghu1/DATA/MeteoNet_2018/站点分布图.npz')
data = {key: data[key] for key in data}

dir0 = r'/mnt/hdd1/zhanghonghu1/DATA/MeteoNet_2018/v5_nor'
paths = os.listdir(dir0)

# 创建保存补全后数据的目录
save_dir = r'/mnt/hdd1/zhanghonghu1/DATA/MeteoNet_2018/v5_nor_buquan'
...
for path in paths:
    ...
    # 找出缺失值的站点索引
    missing_indices = []
    for i, full_coord in enumerate(full_coords):
        found = False
        for j, available_coord in enumerate(available_coords):
            if np.allclose(full_coord, available_coord):
                found = True
                break
        if not found:
    ...
    # 缺失值站点的经纬度坐标
    missing_coords = full_coords[missing_indices]
    ...
    if len(missing_coords) > 0:
        interpolated_temps = kriging_interpolation(station_data, missing_coords)
    else:
        interpolated_temps = []

    # 合并已有值和插值结果
    final_temps = np.zeros(len(full_coords))
    for i, available_coord in enumerate(available_coords):
        for j, full_coord in enumerate(full_coords):
            if np.allclose(full_coord, available_coord):
                final_temps[j] = available_temps[i]
                break
    for i, index in enumerate(missing_indices):
        final_temps[index] = interpolated_temps[i]

    # 将插值结果保存到x1中
    x1['t'] = np.array(final_temps)
    x1['lat'] = data['lat']
    x1['lon'] = data['lon']
    x1['id'] = data['id']
